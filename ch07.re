...
ITS を構成したソフトウェアコードがそうであるように、ITS 廃止の起源も、それ以前の時期に遡ることができる。1980 年までに、ラボの大半のハッカーは、Lisp マシンとそのオペレーティング・システムの開発に従事していた。
...
「MIT の AI ラボに初めてパスワードが登場した（ときに）、僕は自分の信念に従ってパスワードなしにする（ことを決めました）」と後に、ストールマンは語っている。「コンピュータにセキュリティを施すのが本当に望ましいとは信じませんでしたから、ぼくがセキュリティ体制を進んで手助けする筋合いはなかったのです。」@<fn>{12}
...
最初、他のハッカーたちは彼らの時間の一部を依然として MIT で使い、MIT の Lisp マシン・オペレーティングシステムに貢献した。LMI 社もシンボリクス社も、このコードを MIT からライセンスされていた。ライセンスは、両者が加えた変更について MIT に報告することを求めていたが、MIT にこれらの変更を再配布させることを求めていなかった。しかし、1981 年までは、それを許諾するという紳士協定が守られていて、彼らによる全てのシステムの改良は MIT ヴァージョンに含まれたので、Lisp マシンの全ユーザーがそれを共有できた。この状況のおかげで、MIT に残った者も中立的でありえた。
...
1960 年代全部を 1950 年代への回帰に費やした人間として、ストールマンは同僚と歩調が合わないことを気にかけなかった。しかし、ストールマンは、最良のマシンと最良のソフトウェアで仕事をしてきたプログラマとして、彼が「截然とした道徳的選択」と表現するほかないものに直面していた。すなわち、倫理的に「占有的」ソフトウェアに反対することをあきらめるのか、それとも、非占有的なソフトウェア・プログラムのシステムという別の選択肢の構築に自分の人生を捧げるのか、という選択である。占有的（proprietary；プロプライエタリ）ソフトウェアとは、ストールマンや彼の仲間のハッカーが、コピーや修正を制限する著作権条項やエンドユーザーライセンス持つ一切のプログラムを表すために使っている用語である。シンボリクス社との2年間の戦いを経た後、ストールマンには後の選択肢を引き受ける自信があった。「コンピュータの仕事をすっかりやめてしまうこともできたと思います」とストールマンは言う。「ぼくには特別な技能はありませんでしたが、ウェイターになることはできたと思います。たぶん、しゃれたレストランではないでしょうが、どこかのウェイターにはなれたでしょう。」

ウェイターをする、つまり、プログラミングから完全にドロップアウトすることは、彼に多くの喜びを与えてきた活動であるコンピュータ・プログラミングを、完全に諦めることを意味しただろう。ケンブリッジに引っ越して来てからの人生を振り返って、ストールマンはソフトウェア・プログラミングが唯一の喜びを与えてくれた多くの時を、容易に見分けることができることに気づく。ドロップアウトするより、ストールマンはプログラミングをやり抜くことにした。
...
聴衆に語りかけるとき、ストールマンは、宗教的なアプローチを避け、プラグマチックな言葉でその決心を表現する。「自問してみました。一人のオペレーティング・システムの開発者が、状況を改善するために何ができるか、と。この疑問をしばらく吟味してみると、この問題を解決するのに必要とされているのは、まさに一人のオペレーティング・システム開発者だと気付きました。」
...
辞職は、ストールマンの仕事を MIT の法的庇護から遠ざけた。それでも、ストールマンは、MIT の施設を使い続けるとともに、後に自分のオフィスを保有するのに十分な友人と同盟者を AI ラボの中に持っていた。また彼は、GNU プロジェクトの初期段階の費用を賄うために、外部からのコンサルタント業務を確保する能力も持っていた。他方、MIT を辞めたことで、ストールマンは、利益相反関係やソフトウェアに対する研究所の所有権をめぐる議論を無効にした。大人になりたての頃、社会的孤立への恐れから AI ラボの抱擁の中へますますのめりこんでいった男が、今は彼自身とその環境との間に法的な防火壁を築くようになっていた。
...
当初、ストールマンは、オリジナルの PDP-10 Emacs との完全な互換性を実装するのに、ユーザーレベルのコマンドだけを変更しようと思っていた。しかし、本物の Lisp と比べて Mocklisp の非力さが分かると、彼はそれを本物の Lisp システムと置き換えざるをえないと感じた。これは当然ながら、Gosmacs のハイレベルコードの大半を、Lisp のより強力な力と柔軟なデータ構造を生かして、全く違ったやり方で書き直させるものだった。1985年の中頃までにインターネットにリリースされた GNU Emacs では、Gosmacs 由来のコードは少しのファイルに残っているだけだった。
...
「褒賞に値するものがあるとすれば、それは社会的な貢献である」とストールマンは書いた。「創造性は社会的貢献でありうる。しかし、それは社会がその結果を自由に使える限りにおいてのことだ。プログラマが革新的なプログラムの創造により褒賞に値するとすれば、同じ理由により、これらのプログラムの使用を制限するならば、処罰に値する。」@<fn>{21}
...
「85年の支出と収入は、総額でおよそ 23,000 ドル程度だったと思います。」とシャッセルは回想する。「リチャードには自分のオフィスがあったので、我々はそこに場所を借りました。私は何でもかんでも、とくにテープは、私のデスクの下に置いていました。しばらくして、テープやその手のものを保管できる場所を LMI 社が貸してくれるまでそうしていました。」

自由ソフトウェア財団は、顔の提供に加えて、幻想から醒めた他のプログラマたちのための重心も提供した。ストールマンが最初に GNU を発表をしたときでさえ非常に和気あいあいとしているように見えた Unix 市場は、ますます競争的になっていった。顧客を繋ぎ止めようとして、会社はユーザの Unix のソースコードへのアクセスを拒否し始めていた。この流れは進行中の GNU ソフトウェア・プロジェクトへの問合せの数を激増させた。かつてストールマンをやかまし屋の変人だと思っていた Unix ウィザードたちは、今や自分で確かめた問題を逃れることに希望を感じるか絶望を感じるかに応じて、ストールマンをソフトウェアの預言者あるいはカサンドラ[訳注：凶事の予言者]と見始めるようになった。
...
//footnote[4][Marshall Kirk McKusick, "Twenty Years of Berkeley Unix," Open Sources (O'Reilly & Associates, Inc., 1999): 38.『バークレー Unix の 20 年』マーシャル・カーク・マクージック、「オープンソース」（株式会社オライリー・ジャパン、1999年）を参照]
...
//footnote[15][『ハッカーズ』で、スティーブン・レヴィがこの時期を念頭に、ストールマンを「最後の真のハッカー」と記述したとき、彼の意図した意味はあなたが思うものと違うかもしれない。レヴィは「真のハッカー」という用語を MIT のハッカー・コミュニティを他の二つのハッカー・コミュニティから区別するのに使っている。後者は本の後の方で説明されており、別の名前が与えられている。[訳注;「真のハッカー」は第一部のタイトルでMITハッカーを指している。第二部はハードウェア・ハッカー、第三部はゲームハッカーである。\]このコミュニティがストールマンを一人残して溶解したとき、彼は結果的に最後の「真のハッカー」になった。レヴィは、他の者は誰も本当のハッカーではなかったと言おうとしているのではない。しかし、人々は、特にレヴィの本の説明を読んでいない人々は、彼の言葉をそのように解釈する傾向がある。ストールマンが、レヴィの言葉で自分を形容したことはない。]
...
//footnote[19][Richard Stallman, "The GNU Operating System and the Free Software Movement," Open Sources (O'Reilly & Associates, Inc., 1999): 65.「GNU システムとフリーソフトウェア運動」、リチャード・ストールマン、『オープンソース』（株式会社オライリー・ジャパン、1999 年）を参照。]
