...
GNU Emacs ライセンスを作るにあたって、ストールマンはかつての Emacsコミューンの非公式な主義に大きな変更を一つ加えた。かつてはコミューンメンバーに彼らが書いたすべての変更を送るように要求した個所で、今やストールマンは、プログラムの再配布を選択したときには、必ずソースコードと自由の告知をすることを要求しただけだった。言い替えれば、自家用の修正を Emacs に加えただけのプログラマは、もうストールマンにソースコードの変更を送り返す必要がなかった。自由ソフトウェアの教義のめったにない変更にあたって、ストールマンは、自由ソフトウェアの値段を切り下げることにした。どのコピーにも所持者がコピーをさらに開発して再配布する許可が付いている限り、ユーザーは肩越しに覗くストールマンぬきでイノベーションをすることができ、彼らが望むときにだけ彼らの修正版を再配布してよいことになった。
...
もっと一般的なアプローチをするように提案してきたのはギルモアだけではなかった。1986 年の暮までに、ストールマン自身、GNU プロジェクトの次の大きな一里塚であるソースコードデバッガー、GDB にとりかかっていた。このリリースのため、彼は、GNU Emacs にかわって GDB に適用できるように Emacs ライセンスを修正する必要があった。大仕事ではなかったものの、それは間違えるかもしれない仕事の開始だった。1989 年、ストールマンは Emacs への個別的言及を除く方法を解決して、プログラム・コードをプログラムのソースファイルの中にあるライセンスとだけ結合することを表明した。このやり方なら、どのプログラマもライセンスを変更せずに、自分のプログラムにライセンスを適用することができた。GNU一般公衆ライセンス（The GNU General Public License）、略してGPLの誕生だった。GNUプロジェクトは、まもなくそれを既存の全てのGNUプログラムの公式ライセンスにした。
...
ハッカーは著作権法を疑わしいものと見る代りに、それをハックされたがっているもうひとつのシステムと見るべきだ、ということだ。
...
その公開以来、他の点ではストールマンに少しも忠誠心を表明しないプログラマや企業が GPL の取引を額面通りに喜んで受け入れてきた。自身のソフトウェアプログラムの先制的防御メカニズムとして GPL を受け入れる者さえいる。GPL の条件を制約が強すぎるとして拒絶する者でさえ、その影響力を認める。
...
1980 年代終りには、GPL は、自由ソフトウェアのコミュニティに重力効果を及ぼし始めた。BSD ユーティリティの事例に見られるように、自由ソフトウェアと認められるのにライセンスを GPL にする必要はなかったが、プログラムを GPL の下に置けば明確なメッセージが送られることになる。「GPL があったから、みんなが自分たちが作っているのが自由なソフトウェアなのかどうか、どんなライセンスにするか、をとことん考えるようになったと思う」とブルース・ペレンス（Bruce Perens）は言う。
...
そういう営利的利用は、自由ソフトウェアの課題と完全に調和している。
...
最小のソフトウェア会社でも、世界の IBM や DEC と自由に競争できる状況が生まれた。
...
1980 年代のプログラマは偉大なアメリカのコンパイラを書くことについて語っていたんだ」とティーマンは回想する。「突然、ストールマンはそれをやってしまった。恐れ入った。」
...
ティーマンは、GNU 宣言の中でさらにインスピレーションを感じた。
宣言は、占有的ソフトウェアベンダーの貪欲を激しく非難する一方で、会社が
消費者の視点から自由ソフトウェアの利点を考え、企業活動にあたって自由ソフトウェアを使用し、再配布することを奨めていた。GPL は、商業的ソフトウェアの問題から独占の力を取り除くことによって、
小さな会社でもサービスに基づいて競争できるようにする。サービスは単純な技術支援から個別の依頼者の必要に応じた自由なプログラムの拡張のトレーニングまで広がっている。

1999 年のエッセーで、ティーマンはストールマンの宣言の衝撃を回想している。
「それは社会主義的議論のようにも読めるが、別の重要なことだと分かった。
見方を変えたビジネスプランでした。」@<fn>{7}

このビジネスプランは新しいものではなかった。80 年代後半に、ストールマンは小規模にこれをやることで自分の生活を支えていた。しかし、ティーマンはそれを新しいレベルに持って行こうとしていた。ティーマンはジョン・ギルモアとデヴィッド・ウォリス（David Vinayak Wallace）とチームを組み、GNU プログラムのカスタマイズに特化したソフトウェアのコンサルティングサービスを立ち上げた。シグナスサポート（Cygnus Support）と称するその会社が（ "Cygnus" は、非公式的に、"Cygnus, Your GNU Support" の再帰的頭字語だった）、最初の開発契約にサインしたのは 1990 年 2 月のことだ。その年の暮には、会社はサポートと開発契約で 725,000 ドル相当の財産を手にしていた。
...
プログラムをより柔軟にしていく過程で、ストールマンと彼の協力者たちはしばしばそれらをより実用的なものにした。

GNU の普遍主義的アプローチを回想して、プライムタイム・フリーウェアのモーリンは、GNU Hello という非実用的だが極めて重要なソフトウェアパッケージを指摘する。それはプログラマーにとって GNU のためのプログラムの正しいパッケージの方法のお手本として役立つもので、
...
ストールマンによれば、自由ソフトウェアに置き換えることが第一で、
Unix のコンポーネントの技術的改良は二義的なことだった。
「各構成要素について、ぼくはそれを改良する方法を見つけることもあるかもしれませんし、
見つけないこともあるでしょう」とストールマンは Byte 誌に語っている。
「ぼくは、ある程度まで再実装から利益を得ています。再実装すると大抵のシステムはずっと良くなるんです。それは、ある程度まで、ぼくがこの分野に長いこといて、他の多くのシステムでも仕事をしてきたからだと思います。だから、[そこから学んだ]沢山のアイデアを持ち込めるんです。」@<fn>{8}
...
自由ソフトウェアのオペレーティングシステムを造ることを追求しながら、未だに実現されていないために、流行と成功が一番の価値観になっている人々からすれば、GNU プロジェクトは絶望的に時代遅れに見えた。
ストールマンが「ルックアンドフィール」訴訟との戦いに献身するまったく新しいグループを集める必要性を感じている事実そのものが、一部の観察者の目には FSF は時代遅れだと考えさせるものに映った。

しかしながら、ストールマンには戦略的理由があった。ソフトウェア開発に対する独占企業の新たな押し付けには別の組織をスタートさせて戦うのだ。占有的ソフトウェアの開発者もそれに加わるだろう。著作権をインターフェースをカバーするものに拡大することは、多くの自由ソフトウェア開発者と並んで多くの占有的ソフトウェア開発者を脅威にさらす。これらの占有的ソフトウェア開発者が自由ソフトウェア財団を支持することはありそうにないが、プログラミングの自由同盟には、ことさら彼らを追い払うようなものは何もなかった。同じ理由から、それがものになりそうになると、ストールマンはプログラミングの自由同盟のリーダーシップをすぐに手放した。
...
ストールマンの課題の「社会政治的な」側面はピンとこなかったが、トーバルズは課題の基底をなす論理、エラーのないコードを書くプログラマなんていない、を高く評価した。特定の好みに合わせてプログラムを修正するユーザーの要望がないときでさえ、いかなるプログラムも改良できるのである。
ソフトウェアを共有することで、貪欲やエゴの防衛といった個人的動機よりも、ハッカーはプログラムの改良を優先するのだ。
...
トーバルズは Minix（ミニックス）を使っていたが、それはオランダの大学教授、アンドリュー・タネンバウム（Andrew Tanenbaum）が教育的手本として開発した自由のない軽量システムだった。@<fn>{14} それは自由のない自由大学コンパイラ・キットに加えて、タネンバウムが 1983 年の手紙でストールマンが書くように軽蔑的に招待した類いのユーティリティを含んでいた。@<fn>{15}
...
Linux を GCC でコンパイルしても、Linux で GCC を走らせても、GNU GPL で Linux をリリースする法的必要性はなかったが、トーバルズが GCC を使っていることは、他のユーザーにそれを借用させるあの責任を含意していた。トーバルズが後に述べたように、「ぼくは巨人の肩の上に乗っていた」のだった。20 彼が、他の人が同じような支援のために彼をあてにしてきたときに何が起きるか考え始めたのは驚くに当たらない。
決断の十年後、トーバルズは自由ソフトウェア財団のロバート・シャッセルに、
当時の考え方を要約して返している。
...
1994 年までに、この寄せ集めのオペレーティングシステムはハッカー界で十分な尊敬を勝ち得ていたが、観察者の中には、プロジェクトの最初の数ヶ月のうちにトーバルズが GPL に切り替えることで、農園を手放すようなことをしていなかったらどうなっていただろう、と思う者もいた。リナックスジャーナルの創刊号で発行者のロバート・ヤングは、トーバルズとのインタビューの席についていた。ヤングがこのフィンランド人プログラマに、リナックスのソースコードの私的所有権を放棄したことを後悔していないのかと聞くと、トーバルズはしていないと答えた。
...
//footnote[6][カリフォルニア大学の「うざったい広告条項」は、その後、問題があることが明らかになった。ハッカーの中には、GPL ではない、自由放任な代替物を求めて、カリフォルニア大学のライセンスを「カリフォルニア大学」というところだけ自分自身や自分の組織の名前に置き換えて使っていた者もいた。その結果、これらのプログラムを多数使った自由ソフトウェア・システムは、広告で数十の名前を引用する必要があった。1999 年、ストールマン側の数年がかりの働きかけを経て、カリフォルニア大学はこの条項を削ることに同意した。BSD ライセンスが抱える問題を参照。 http://www.gnu.org/philosophy/bsd.ja.html ]

//footnote[7][Michael Tiemann, "Future of Cygnus Solutions: An Entrepreneur's Account," Open Sources (O'Reilly & Associates, Inc., 1999): 139 マイケル・ティーマン、「シグナスソリューションズ社の将来性」『オープンソース』（1999年、オライリー・ジャパン）143 ページを参照。]
