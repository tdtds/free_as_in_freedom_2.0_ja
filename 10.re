...
「興味をかきたてよう」として、マードックは自分の考えを、ユーズネットの comp.os.linux ニュースグループなど、インターネット上に投稿した。最初の返信メールの一つは、<a href="mailto:rms@ai.mit.edu" rel="nofollow">rms@ai.mit.edu</a> からのものだった。ハッカーであるマードックは、即座にそのアドレスが誰のものかを理解した。それは GNU プロジェクトの創設者リチャード・M・ストールマンであり、マードックが以前から「ハッカーのなかのハッカー」として知っていた人物だった。メールの一覧にそのアドレスを見て、マードックは戸惑った。いったいなぜストールマン、つまり自分のオペレーティング・システムのプロジェクトを率いている人物が、マードックの &quot;Linux&quot; ディストリビューションに対する不満を気にかけるのだろう？
...
1 ここで引用したような発言を、トーバルズは複数の機会に行っている。だが、今のところ、この引用で一番有名なのは、エリック・レイモンドのエッセー『伽藍とバザール』（1997年5月）の中のものだ。<a href="http://catb.org/esr/writings/homesteading/cathedral-bazaar/" rel="nofollow">http://catb.org/esr/writings/homesteading/cathedral-bazaar/</a>（日本語訳： <a href="http://cruel.org/freeware/cathedral.html" rel="nofollow">http://cruel.org/freeware/cathedral.html</a>）
...
GNU/Linux が、他の自由ソフトウェアのオペレーティング・システムに対して、相対的に成功したことの分析例としては、ニュージーランドのハッカー、リアム・グリーンウッド（Liam Greenwood）のエッセー『Linux 成功の理由』（1999 年）を参照。<a href="http://www.freebsddiary.org/linux.php" rel="nofollow">http://www.freebsddiary.org/linux.php</a>
...
<blockquote><p class="quotation">「タイミングエラー」は非同期システムの中で発生します。非同期システムでは、並行して実行されるジョブは、理論的には任意の順序で起動可能ですが、ある特定の順序で起動されたときに問題が発生します。プログラム A が行う処理を X、プログラム B が行う処理を Y とし、X も Y も同一のデータ構造を検査して更新する短い処理だとしましょう。ほとんどの場合、コンピュータは、Y の前に X を行うか、X の前に Y を行うかで問題は起こりません。だが、稀に、たまたまスケジューラがプログラム A を実行し、処理 X の途中で、処理 Y を行うプログラム B を実行する場合があります。すると、Y は X の途中で実行されることになる。これらは同じデータ構造を更新するので、互いに影響し合うのです。例えば、X はおそらくデータ構造の検査を終っていて、変更があったことを気づかないでしょう。そこで、再現しづらいエラーが発生することになる。再現しづらいのは、偶然の要因に（スケジューラがどのプログラムをどれだけの時間実行するかを決定するときのような）左右されるからです。こうしたエラーを回避するには、X と Y が同時に実行されないことを保証するために、ロックを用いる方法があります。非同期システムを書くプログラマーは、ロックの一般的な必要性は理解しているのですが、ときどき、特定の場所やデータ構造で必要なロックを見過ごします。すると、そのプログラムにはタイミングエラーが存在することになるのです。
</blockquote>
5 GNU/Linux のユーザ人口は、せいぜい概算しか手に入らないため、こんなに範囲が広くなっている。10 万人という数字は、レッドハットの『マイルストーン』からのもの。<a href="http://www.redhat.com/about/corporate/milestones.html" rel="nofollow">http://www.redhat.com/about/corporate/milestones.html</a>

6. このようにウィンストン・チャーチルになぞらえたのは、ストールマン自身がチャーチルに関する次のコメントを、頼まれないのに送ってくれたときよりも前のことである。

<blockquote><p class="quotation">第二次世界大戦とその勝利に必要とされた決意とは、ぼくの成長期の強烈な思い出だ。「我々は上陸地点で彼らと戦うのだ、浜辺で彼らと戦うのだ . . . 我々は絶対に降伏しない」というようなチャーチルの演説は、いつもぼくの中で鳴り響いていた。
</blockquote>
7 イアン・マードック『Debian 小史』(1994年1月6日)：付録 A、『Debian 宣言』。<a href="http://www.debian.org/doc/manuals/project-history/apA.html" rel="nofollow">http://www.debian.org/doc/manuals/project-history/apA.html</a>

8. 元 Lucid のプログラマーで、後に Mozilla 開発チームを率いることになる、ジェイミー・ザウィンスキー（Jamie Zawinski）のウェブサイトに、Lucid/GNU Emacs のフォークに関するドキュメントがある。タイトルは、『Lemacs/FSFmacsの分裂』。<a href="http://www.jwz.org/doc/lemacs.html" rel="nofollow">http://www.jwz.org/doc/lemacs.html</a>
