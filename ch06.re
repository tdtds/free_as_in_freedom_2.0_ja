...
寮を失ったが、ニューヨークに戻るつもりもなく、ストールマンは、グリーンブラット、ゴスパー、サスマンや彼以前の多くの他のハッカーたちが切り開いた道をたどっていった。MIT の大学院生になり、ストールマンはケンブリッジの近くにアパートの一室を借りたが、やがて AI ラボそのものを彼の事実上の我が家とみなすようになった。1986 年の講演で、ストールマンはこの頃の AI ラボの記憶を呼び戻している。
...
TECO は旧式のエディタよりかなり改善されていたが、まだ欠点があった。文書を作成して編集するのに、プログラマは 1 回の編集を指定する毎に、一連のコマンドを入力しなければならなかった。これは抽象的な手続きだった。キー入力の度に文章を更新していく現在のワープロとは異なり、文章を変更するだけでも、TECO は、ユーザに拡張された一連の編集コマンドと「コマンドの終り」を表す一連の文字を入力することを要求した。しばらくすると、ハッカーは大きな変更を 1 つのコマンド文字列で行うまでに習熟していったが、後にストールマン自身が指摘したように、この手順は「目隠ししてチェスをするのに匹敵する精神的技量」を要した。@<fn>{5}
この手順を軽減するために、AI ラボのハッカーたちは、画面を分割してテキストとコマンド文字列を表示するシステムを造った。しかし、こうした革新的なハックによっても、TECO で編集するには熟練と計画を要した。
...
このハックに影響されたストールマンは、MIT に帰ると、TECO の機能を同様な仕方で拡張する方法を探した。彼はカール・ミケルソン（Carl Mikkelson）が書き、それを起動する二つのキー入力の組合せから命名した、Control-R という TECO の機能を見つけた。ミケルソンのハックは、TECO を通常の抽象的なコマンド実行モードから、より直感的なキー入力ベースのモードに切り替えるものだった。その唯一の欠点は、それが画面を 5 行分しか使わず、実際に使うにはあまりに非効率だったことだ。ストールマンはこの機能が画面全体を効率よく使用するように実装し直し、次にこの機能を繊細かつ大胆に拡張した。
彼は TECO のコマンド文字列つまり「マクロ」をキーに割り当てられるようにした。
上級の TECO ユーザは、以前からマクロをファイルに保存していた。
ストールマンのハックにより、その呼び出しが素早くできるようになった。
その結果、ユーザによるプログラム可能な WYSIWYG エディタができあがった。
「これが真の突破口でした」とその当時の同僚で AI ラボのハッカーだったガイ・スティール（Guy Steele）は言う。@<fn>{7}
...
この爆発から 2 年後、発明の勢いが不具合な副作用を生み始めた。この爆発的な成長は、ハッカーの協調的アプローチのわくわくするような有効性を示したが、非互換性をもたらすようにもなった。「バベルの塔効果が現れたんです」とガイ・スティールは言う。
...
このプロジェクトの新しい名前、Emacs は、ストールマンに負っている。Emacs は、「編集用マクロ（editing macros）」の省略記法だが、これは 2 年前にマクロが爆発したときに起った進化上の卓越性を含意していた。そしてそれは、ソフトウェアプログラムの名前辞書の隙間を利用したものでもあった。ITS のプログラムに文字 "E" で始まるものが存在しないことに気づいて、ストールマンは Emacs を選んだ。こうすると、自ずからプログラムを 1 文字で参照するようになる。またしても、ハッカーの効率性への情熱が、その痕跡を残したことになる。@<fn>{10}
...
オリジナルの Emacs は、PDP-10 コンピュータ上でのみ動いたが、まもなく他のコンピュータのユーザーも Emacs で編集することを欲した。爆発的イノベーションが十年間続き、数多くの Emacs 風プログラムが生まれたが互換性はまちまちだった。Emacs コミューンのルールはそれらに適用されなかった。それらのコードは分離されていたからだ。その幾つかは、ストールマンのオリジナルの Emacs との関係をユーモラスな再帰的名称で引用した。Sine（Sine is not Emacs; SineはEmacsに非ず）、Eine（Eine is not Emacs; EineはEmacsに非ず）、Zwei（Zwei was Eine initially; Zweiは始めEineだった）。オリジナルがそうだったように、本物の Emacs はユーザーのプログラミング可能性を提供しないといけなかった。類似のキーワードコマンドを持つが、ユーザがプログラム可能でないものは「作り物の」Emacs と呼ばれた。例えば、Mince（Mince is Not Complete Emacs; Minceは完全なEmacsに非ず）がそれだ。

AI ラボではストールマンが Emacs を開発している一方で、他の所では、ハッカーコミュニティを不安にさせる開発があった。1979 年にブライアン・リードが スクライブに「時限爆弾」を仕込み、ユニロジック社がソフトウェアへの未払いユーザーのアクセスを制限することを可能にしたのは、ストールマンにとって悪い前兆だった。「彼はそれを生涯で目撃した最もナチス的な出来事だと考えていました」とリードは回想する。リードはユーズネットの alt 階層の共同創始者として、後日インターネットで名声を博することになったが、それでも、彼は、1979 年にしたことの償いは未だ終っていない、少くともストールマンの目から見れば、と言う。「彼は全てのソフトウェアは自由でなければならない、また、ソフトウェアに課金するような企ては人類に対する犯罪だと言っていました。」@<fn>{12}

ストールマンにはリードの取引きを食い止める力はなかったが、ハッカー精神に悖ると思われる他の行動形態を抑えることはできた。オリジナル Emacs の中心的なコードメンテナとして、ストールマンはその権力を振るって政治的効果をもたらそうとし始めた。パスワードをめぐるコンピュータ科学研究所の管理者とのいざこざの最終段階で、ストールマンはソフトウェア「ストライキ」を開始し、研究所のコンピュータのセキュリティシステムを拒否するまで、研究所のメンバーに Emacs の最新版を送付するのを断った。@<fn>{13} これは制裁措置というより、ジェスチャーだった。なぜなら、彼らが自分でインストールするのを止めさせるものは何もなかったのだから。しかし、真意は伝わった。ITS システムにパスワードをつけることは、非難と反発をもたらすのだ。
...
//footnote[3][Joseph Weizenbaum, Computer Power and Human Reason: From Judgment to Calculation (W. H. Freeman, 1976) （ジョセフ・ワイゼンバウム『コンピュータ・パワー―人工知能と人間の理性』、サイマル出版会、1979年、秋葉忠利訳）を参照。]
...
//footnote[11][ストールマン（1979年）: #SEC34 を参照。[訳注: 注6で参照しているメモの脚注に、この配布に関する文章が見られる。\]]
